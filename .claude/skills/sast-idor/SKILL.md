...
>
> **Deep Internal Call Tracing — follow ALL project-defined functions when verifying authorization**
>
> When a middleware or helper function is referenced in an authorization check, read its full implementation:
> 1. Look up the function definition. Is it defined in this project (not in `node_modules/`)?
> 2. If YES: read that function's source code. Follow the authorization logic recursively through all project-defined helper calls.
> 3. Continue until you reach one of three terminal conditions:
>    - (a) An explicit ownership/permission check against `req.user` / current user
>    - (b) A `node_modules` import boundary — the logic is delegated to a library
>    - (c) The function returns without performing any ownership check
> 4. Document each hop, e.g.:
>    `checkAccess() → getPermissions() → verifyOwner(resource, userId)` — or confirm where the chain ends without a check.
>
> Do NOT assume a function named `auth` or `authorize` actually checks resource ownership. Read its source and trace through all layers until you confirm whether ownership is verified.
>
