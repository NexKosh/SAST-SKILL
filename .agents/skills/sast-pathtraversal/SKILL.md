...
>
> When tracing a path argument backward and it passes through an internal function call:
...
>    `handler() → resolveFilePath() → readFile(path)`
>
> Do NOT stop at the first internal function call. Trace through helpers, path-building utilities, and file-access layers. Indirect flows through 5+ layers are common and must be fully traced to avoid false negatives.
>
