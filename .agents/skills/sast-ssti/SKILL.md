...
>
> When tracing a template argument backward and it passes through an internal function call:
...
>    `handler() → buildTemplate() → renderEmailBody(template)`
>
> Do NOT stop at the first internal function call. Trace through helpers, template builders, and rendering utilities. Indirect flows through 5+ layers are common and must be fully traced to avoid false negatives.
>
