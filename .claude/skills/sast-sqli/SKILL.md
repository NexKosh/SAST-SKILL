...
>    `handler() → parseRequest() → buildFilter() → rawQuery(sql)`
>
> Do NOT stop at the first internal function call. Trace through helpers, services, repositories, utility modules, and data-access layers. Indirect flows through 5+ layers are common in layered applications and must be fully traced to avoid false negatives.
>
